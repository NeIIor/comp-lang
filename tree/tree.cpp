...
void print_tree (bin_tree_elem *element, FILE *output)
{
    ...
    fprintf(output, "[%p] type = %d, value = %lg, left = [%p], right = [%p]\n", (void *) element, element->type, element->value, (void *) element->left, (void *) element->right);
...
void declare_vertex_tree (bin_tree_elem *element, FILE *output)
{
    if (element->left == nullptr && element->right == nullptr)
        fprintf(output, "  \"%p\" [shape = \"record\", fillcolor = \"red\", label = \"{value: %lg | <f0> pos\\n%p| left\\n%p | right\\n%p | type: %d\\n | size: %d | hash: %d}\"];\n", 
               (void *) element, element->value, (void *) element, (void *) element->left, (void *) element->right, element->type, element->elem_size, element->hash_tree);
    else
...
void link_vertex_tree (bin_tree_elem *element, FILE *output)
{
    if (element->left != nullptr)
    {
        fprintf(output, "  \"%p\"->\"%p\";\n", (void *) element, (void *) element->left);
    ...
    if (element->right != nullptr)
    {
        fprintf(output, "  \"%p\"->\"%p\";\n", (void *) element, (void *) element->right);
